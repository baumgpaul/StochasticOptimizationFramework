function [problemConfig] = cfgProblemSchaffer4
% loads Problem config for schaffer4 foo
%
...
function varargout = schaffer4Fun(pop)
   varargout{1} = 0.5+(cos(sin(abs(pop(:,1).^2-pop(:,2).^2))).^2-0.5)./((1+0.001.*(pop(:,1).^2+pop(:,2).^2)).^2);
