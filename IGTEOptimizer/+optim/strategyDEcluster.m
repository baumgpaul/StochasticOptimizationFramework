function [optim] = strategyDEcluster(optim)
...
while 1      % Start iteration steps
    ...
    for i=1:nSwarm
        [k,dist_individuals] = dsearchn(currIter.population(i,:),currIter.population);
        [nearest_value nearest_ind] = sort(dist_individuals);
        nearest(i)=nearest_ind(2);
    end
    v(1:nSwarm,1:nPar)=currIter.population(nearest',1:nPar) + F*(currIter.population(permat(1:nSwarm,1),1:nPar)-currIter.population(permat(1:nSwarm,2),1:nPar));
